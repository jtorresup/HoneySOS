...
export default function TicTacToe() {
  ...
  // Game history
  const [moveHistory, setMoveHistory] = useState<{ player: Player; position: number; moveNumber: number }[]>([])
  ...
  return (
    <div className="w-full max-w-6xl mx-auto h-screen overflow-y-auto bg-yellow-100 p-4 font-sans">
      <div className="bg-yellow-400 rounded-t-xl p-4 text-center font-bold text-2xl text-black shadow-md">
      ...
      <div className="flex justify-around mt-4 flex-wrap gap-2">
        ...
        {/* Control Buttons */}
        <div className="space-x-2">
          <button className="bg-white text-black px-4 py-2 rounded font-bold border-2 border-black" onClick={resetGame}>
            New Game
          </button>
          <button
          ...
          {gameMode === "computer" && (
            <div className="text-center mb-4 text-sm">
              <div className="bg-white border-2 border-black rounded p-2 inline-block">
                <span className="text-red-600 font-bold">X = {firstPlayer === "computer" ? "Computer" : "Human"}</span>
                {" | "}
                <span className="text-blue-600 font-bold">O = {firstPlayer === "computer" ? "Human" : "Computer"}</span>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-2 max-w-md mx-auto">
            {board.map((cell, index) => (
              <button
                ...
                disabled={!!cell || !!winner || !isGameActive || (gameMode === "computer" && isComputerTurn())}
              >
                <span className={cell === "X" ? "text-red-600" : "text-blue-600"}>{getCellValue(index)}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="bg-yellow-300 border-2 border-black rounded-lg p-2 mt-4 mb-4">
        ...
        <div className="p-2 bg-white border-2 border-black rounded mt-2">
          <div className="text-sm space-y-2">
            <p>
              <strong>Objective:</strong> Be the first player to get three of your marks (X or O) in a row, column, or
              diagonal.
            </p>
            <p>
              <strong>Gameplay:</strong> Players take turns placing their marks on the 3×3 grid. X always goes first.
            </p>
            <p>
              <strong>Winning:</strong> The first player to align three marks wins. If all 9 squares are filled without
              a winner, it's a draw.
            </p>
            <p>
              <strong>Computer Mode:</strong> Play against AI with three difficulty levels. You can choose who goes
              first - Human or Computer.
            </p>
            <p>
              <strong>First Player:</strong> In computer mode, you can select whether the Human or Computer goes first
              (plays as X).
            </p>
          </div>
        </div>
