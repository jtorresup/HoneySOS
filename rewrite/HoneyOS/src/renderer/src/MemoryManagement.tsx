...
export default function MemoryManagement() {
    ...
    const selectNextRRProcess = (waitingProcesses: Process[]) => {
        ...
        if (lastIndex === -1) {
            // If the last process is no longer in the waiting queue, start with the first one
            return sortedProcesses[0]
        }
        ...
        if (!runningProcess && waitingProcesses.length > 0) {
            ...
            if (nextProcess) {
                ...
                const updatedProcesses = processes.map((p) =>
                    p.id === nextProcess!.id ? { ...p, status: "running", startTime: currentTime } : p,
                )
            ...
            const updatedProcesses = processes.map((p) => {
                if (p.id === runningProcess.id) {
                    ...
                    if (selectedAlgorithm === "RR" && currentTime - (p.startTime ?? 0) >= timeQuantum && newRemainingTime > 0) {
    ...
    const calculateWaitingTime = (process: Process) => {
        if (process.endTime !== undefined && process.arrivalTime !== undefined && process.burstTime !== undefined) {
    ...
    return (
        <div className="min-h-screen bg-yellow-100 p-4 font-sans">
            <div className="bg-yellow-400 rounded-t-xl p-4 text-center font-bold text-2xl text-black shadow-md flex items-center justify-between">
                <span>Simulation of the Scheduling Policies</span>
            ...
            <div className="grid grid-cols-4 gap-4 mt-4 p-4">
                {/* Process Table */}
                <div className="bg-yellow-300 border-2 border-black rounded-lg p-2 col-span-3">
                            ...
                            <tbody>
                                {processes.map((process) => (
                                    ...
                                    >
                                        ...
                                        <td className="border border-black p-1 text-center">{process.remainingTime}</td>
                                        <td className="border border-black p-1 text-center">{process.memoryRequired}</td>
                                        ...
                                        <td className="border border-black p-1 text-center">
                                            <span
                                                className={`px-2 py-1 rounded ${process.status === "running"
                                        ...
                                        <td className="border border-black p-1 text-center">{calculateTurnaroundTime(process)}</td>
                                        <td className="border border-black p-1 text-center">{calculateWaitingTime(process)}</td>
                                    </tr>
                                ))}
                                {processes.length === 0 && (
                                    <tr>
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                        <div className="flex flex-col gap-1 mt-2">
                            {memoryBlocks.map((block) => (
                                <div
                                    key={block.id}
                                    className={`h-8 border-2 ${block.processId !== null && getRunningProcess()?.id === block.processId
                                ...
                                >
                                    ...
                                    <span className="text-xs font-bold">{block.processId !== null ? `P${block.processId}` : "-"}</span>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div className="bg-yellow-300 border-2 border-black rounded-lg p-2 mt-4">
                ...
                <div className="grid grid-cols-4 gap-4 p-2">
                    <div className="bg-white p-2 rounded border border-black">
                        ...
                        {selectedAlgorithm === "RR" && <p className="text-center text-sm">Quantum: {timeQuantum}</p>}
                    </div>
                    <div className="bg-white p-2 rounded border border-black">
                    ...
                    <div className="bg-white p-2 rounded border border-black">
                        ...
                        <p className="text-center">{processes.filter((p) => p.status === "running").length}</p>
                    </div>
                    <div className="bg-white p-2 rounded border border-black">
                        <h3 className="font-bold text-center">Completed Processes</h3>
                        <p className="text-center">{processes.filter((p) => p.status === "completed").length}</p>
                    </div>
                </div>
            </div>
        </div>
