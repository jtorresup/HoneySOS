...
export default function ReplacementAlgorithm() {
  // State for pages
  ...
  const findLFUVictim = () => {
    ...
    for (let i = 1; i < memoryFrames.length; i++) {
      ...
      if (currentFreq < minFrequency || (currentFreq === minFrequency && currentTimestamp < oldestTimestamp)) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto h-screen overflow-y-auto bg-yellow-100 p-4 font-sans">
      <div className="bg-yellow-400 rounded-t-xl p-4 text-center font-bold text-2xl text-black shadow-md flex items-center justify-between">
        <span>Simulation of the Replacement Algorithms</span>
      ...
      <div className="mt-4 bg-yellow-300 border-2 border-black rounded-lg p-2">
        ...
        <div className="flex items-center mt-2">
          ...
          <button
            ...
            onClick={(e) => addCustomReference((e.currentTarget.previousSibling as HTMLInputElement).value)}
          >
            Set
          </button>
        </div>
        <div className="mt-2 p-2 bg-white border-2 border-black rounded flex flex-wrap">
          ...
          {referenceString.length === 0 && <span className="text-gray-500">No reference string generated yet.</span>}
        </div>
      </div>

      {/* Page Access History with Memory States */}
      <div className="bg-yellow-300 border-2 border-black rounded-lg p-2 mt-4">
        <h2 className="text-center font-bold border-b-2 border-black pb-1">Page Access History & Memory States</h2>
            ...
            <table className="w-full border-collapse min-w-max">
              <thead>
                <tr className="bg-yellow-400">
                  ...
                  {frameCount > 5 && (
                    <th className="border border-black p-1 w-16 min-w-[64px] text-xs">...+{frameCount - 5}</th>
                  )}
                </tr>
              </thead>
              <tbody>
                {pages.map((page) => (
                  <tr key={page.id} style={{ backgroundColor: page.color }}>
                    ...
                    <td className="border border-black p-2 text-center w-20">{page.referenceString}</td>
                    <td className="border border-black p-2 text-center w-24">
                      <span
                        className={`px-2 py-1 rounded text-xs ${
                          page.status === "hit" ? "bg-green-500 text-white" : "bg-red-500 text-white"
                        }`}
                      >
      ...
      <div className="bg-yellow-300 border-2 border-black rounded-lg p-2 mt-4 mb-4">
        <h2 className="text-center font-bold border-b-2 border-black pb-1">Algorithm Explanation</h2>
        <div className="p-2 bg-white border-2 border-black rounded mt-2">
          {selectedAlgorithm === "FIFO" && (
            <p className="text-sm">
              <strong>First-In-First-Out (FIFO):</strong> The oldest page in memory is replaced when a new page needs to
              be loaded. It's simple but doesn't consider how frequently or recently a page has been used.
            </p>
          )}
          {selectedAlgorithm === "LRU" && (
            <p className="text-sm">
              <strong>Least Recently Used (LRU):</strong> The page that hasn't been used for the longest time is
              replaced. This algorithm works on the principle that pages that have been used recently are likely to be
              used again.
            </p>
          )}
          {selectedAlgorithm === "OPT" && (
            <p className="text-sm">
              <strong>Optimal (OPT):</strong> The page that will not be used for the longest time in the future is
              replaced. This is theoretically optimal but requires future knowledge of page references, making it
              impractical in real systems.
            </p>
          )}
          {selectedAlgorithm === "LFU" && (
            <p className="text-sm">
              <strong>Least Frequently Used (LFU):</strong> The page with the lowest access frequency is replaced. When
              multiple pages have the same lowest frequency, the oldest page (first-in) is chosen as the tie-breaker.
            </p>
          )}
        </div>
      </div>
